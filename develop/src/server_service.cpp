...
// リクエストメッセージを受信する
inline int recvRequestMessage(int sock, char *request_message, unsigned int buf_size) {
...
inline int getProcessing(char *body, char *file_path) {
...
// レスポンスメッセージを作成する
inline int createResponseMessage(char *response_message, int status, char *header, char *body, unsigned int body_size) {
...
// レスポンスメッセージを送信する
inline int sendResponseMessage(int sock, char *response_message, unsigned int message_size) {
